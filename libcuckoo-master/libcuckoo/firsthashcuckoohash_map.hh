...
public:
  ...
  template <typename K> bool erase(const K &key, bool first_hash) {
    return erase_fn(key, [](mapped_type &) { return true; }, bool first_hash);
  }
...
private:
  ...
  table_position cuckoo_find(const K &key, const partial_t partial,
    ...
    int s1 = first_hash ? i1 : i2;
    int s2 = first_hash ? i2 : i1;
  ...
  table_position cuckoo_insert(const hash_value hv, TwoBuckets &b, K &key) {
    ...
    if (res1 != -1) {
      return table_position{i1, static_cast<size_type>(res1), ok};
    }
    if (res2 != -1) {
      return table_position{i2, static_cast<size_type>(res2), ok};
    }
