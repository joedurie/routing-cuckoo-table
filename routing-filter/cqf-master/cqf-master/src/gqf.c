...
uint64_t qf_swap_last_bit(const QF *qf, uint64_t key, uint64_t value, uint8_t flags) {
	return qf_set_last_bit(qf, key, value, flags, 1 ^ qf_get_last_bit(qf, key, value, flags));
}
