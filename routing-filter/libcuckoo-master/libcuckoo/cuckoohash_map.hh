...
public:
   ...
   */
  template <typename K, typename F> bool find_fn(const K &key, F fn, bool first_hash) const {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, first_hash);
    if (pos.status == ok) {
   ...
   * @return true if the key was found and functor invoked, false otherwise
   */
  template <typename K, typename F> bool update_fn(const K &key, F fn, bool first_hash) {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, first_hash);
    if (pos.status == ok) {
   ...
   */
  template <typename K, typename F> bool erase_fn(const K &key, F fn, bool first_hash) {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, first_hash);
    if (pos.status == ok) {
   ...
   */
  template <typename K> bool find(const K &key, mapped_type &val, bool first_hash) const {
    return find_fn(key, [&val](const mapped_type &v) mutable { val = v; }, first_hash);
  }
   ...
   */
  template <typename K> mapped_type find(const K &key, bool first_hash) const {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, first_hash);
    if (pos.status == ok) {
   ...
   */
  template <typename K> bool contains(const K &key, bool first_hash) const {
   ...
   * CopyAssignable.
   */
  template <typename K, typename V> bool update(const K &key, V &&val, bool first_hash) {
    return update_fn(key, [&val](mapped_type &v) { v = std::forward<V>(val); }, first_hash);
  }
   ...
   */
  template <typename K> bool erase(const K &key, bool first_hash) {
    return erase_fn(key, [](mapped_type &) { return true; }, first_hash);
  }
...
private:
  ...
  table_position cuckoo_find(const K &key, const partial_t partial,
                             const size_type i1, const size_type i2, bool first_hash) const {
    size_type s1 = first_hash ? i1 : i2;
    size_type s2 = first_hash ? i2 : i1;
    int slot = try_read_from_bucket(buckets_[s1], partial, key);
    if (slot != -1) {
  ...
  bool cuckoopath_move(QF* qf, const size_type hp, CuckooRecords &cuckoo_path,
    ...
    while (depth > 0) {
      ...
      for(int ind = 0; ind < slot_per_bucket(); ind++)
        qf_swap_last_bit(qf, fb.kvpair(ind).second, fb.kvpair(ind).second, QF_WAIT_FOR_LOCK); 
