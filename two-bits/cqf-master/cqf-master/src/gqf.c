...
static inline int insert1(QF *qf, __uint128_t hash, uint8_t mask, uint8_t runtime_lock)
{
	int ret_distance = 0;
	uint64_t hash_remainder           = (hash & (BITMASK(qf->metadata->bits_per_slot) - 3)) | (mask & 3);
...
static inline int insert(QF *qf, __uint128_t hash, uint64_t count, uint8_t mask, uint8_t
												 runtime_lock)
{
	int ret_distance = 0;
	uint64_t hash_remainder           = (hash & (BITMASK(qf->metadata->bits_per_slot) - 3)) | (mask & 3);
	...
	if (might_be_empty(qf, hash_bucket_index) && runend_index ==
		...
		if (count > 1) {
			insert(qf, hash, count - 1, mask, QF_NO_LOCK);
		}
	} else { /* Non-empty slot */
		...
		if (!is_occupied(qf, hash_bucket_index)) { /* Empty bucket, but its slot is occupied. */
			...
			current_end = decode_counter(qf, runstart_index, &current_remainder,
																	 &current_count);
			while (current_remainder / 4 < hash_remainder / 4 && !is_runend(qf, current_end)) {
			...
			if (current_remainder / 4 < hash_remainder / 4) {
				uint64_t *p = encode_counter(qf, hash_remainder, count, &new_values[67]);
				ret = insert_replace_slots_and_shift_remainders_and_runends_and_offsets(qf, 
																																								1, /* Append to bucket */
																																								hash_bucket_index, 
			...
			} else if (current_remainder / 4 == hash_remainder / 4) {
...
int qf_insert(QF *qf, uint64_t key, uint64_t value, uint64_t count, uint8_t
							flags)
{
	uint8_t mask = qf_get_mask(qf, key, value, flags);
	...
	int ret;
	if (count == 1)
		ret = insert1(qf, hash, mask, flags);
	else
		ret = insert(qf, hash, count, mask, flags);
...
uint8_t qf_get_mask(const QF *qf, uint64_t key, uint64_t value, uint8_t flags)
{
	if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
		if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
...
void qf_set_bit_at_idx(const QF* qf, uint64_t index, uint8_t bit)
{
	set_slot(qf, index, get_slot(qf, index) | (1 << bit));
}

uint64_t qf_set_bit(const QF *qf, uint64_t key, uint64_t value, uint8_t flags, uint8_t bit)
{
	bit = bit & 1;
	...
	do {
		current_end = decode_counter(qf, runstart_index, &current_remainder, &current_count);
		if (current_remainder / 4 == hash_remainder / 4) {
			qf_set_bit_at_idx(qf, runstart_index, bit);
			qf_set_bit_at_idx(qf, current_end, bit);
			return 1;
		}
		runstart_index = current_end + 1;
	} while (!is_runend(qf, current_end));
...
uint64_t qf_swap_mask(const QF *qf, uint64_t key, uint64_t value, uint8_t flags) {
	uint8_t mask = qf_get_mask(qf, key, value, flags);
	if(mask & 1) qf_set_bit(qf, key, value, flags, 1);
	if(mask & 2) qf_set_bit(qf, key, value, flags, 0);
}
