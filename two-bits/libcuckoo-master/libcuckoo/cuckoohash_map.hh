...
public:
   ...
   */
  template <typename K, typename F> bool find_fn(const K &key, F fn, int mask) const {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, mask);
    if (pos.status == ok) {
   ...
   * @return true if the key was found and functor invoked, false otherwise
   */
  template <typename K, typename F> bool update_fn(const K &key, F fn, int mask) {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, mask);
    if (pos.status == ok) {
   ...
   */
  template <typename K, typename F> bool erase_fn(const K &key, F fn, int mask) {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, mask);
    if (pos.status == ok) {
   ...
   */
  template <typename K> bool find(const K &key, mapped_type &val, int mask) const {
    return find_fn(key, [&val](const mapped_type &v) mutable { val = v; }, mask);
  }
   ...
   */
  template <typename K> mapped_type find(const K &key, int mask) const {
    const hash_value hv = hashed_key(key);
    const auto b = snapshot_and_lock_two<normal_mode>(hv);
    const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2, mask);
    if (pos.status == ok) {
   ...
   */
  template <typename K> bool contains(const K &key, int mask) const {
    return find_fn(key, [](const mapped_type &) {}, mask);
  }

  template <typename K> bool containsSingleThreaded(const K &key, int mask) const {
    const hash_value hv = hashed_key(key);
    const size_type hp = hashpower();
    const size_type i1 = index_hash(hp, hv.hash);
    if((mask & 1) && try_read_from_bucket(buckets_[i1], hv.partial, key) != -1) return true;
    const size_type i2 = alt_index(hp, hv.partial, i1);
    if((mask & 2) && try_read_from_bucket(buckets_[i2], hv.partial, key) != -1) return true;
    return false;
  }  
   ...
   * CopyAssignable.
   */
  template <typename K, typename V> bool update(const K &key, V &&val, int mask) {
    return update_fn(key, [&val](mapped_type &v) { v = std::forward<V>(val); }, mask);
  }
   ...
   */
  template <typename K> bool erase(const K &key, int mask) {
    return erase_fn(key, [](mapped_type &) { return true; }, mask);
  }
...
private:
  ...
  table_position cuckoo_insert(QF* qf, const hash_value hv, TwoBuckets &b, K &key) {
    ...
    cuckoo_status st = run_cuckoo<TABLE_MODE>(qf, b, insert_bucket, insert_slot);
  ...
  // will not.
  template <typename TABLE_MODE>
  cuckoo_status run_cuckoo(QF* qf, TwoBuckets &b, size_type &insert_bucket,
    ...
    try {
      while (!done) {
        ...
        if (cuckoopath_move<TABLE_MODE>(qf, hp, cuckoo_path, depth, b)) {
  ...
  template <typename TABLE_MODE>
  bool cuckoopath_move(QF* qf, const size_type hp, CuckooRecords &cuckoo_path,
    ...
    while (depth > 0) {
      ...
      for(int ind = 0; ind < slot_per_bucket(); ind++)
        qf_swap_mask(qf, fb.kvpair(ind).second, fb.kvpair(ind).second, QF_WAIT_FOR_LOCK); 
